       ...
       -- and A.M_MUREX=0 
        ...
        (
           ...
           decode( A.M_SCNTMPL,0,'N','Y')as M_SCNTMPL,
            nvl(SCR.M_TEMPLATE,'') as SCANNER_ENGINE_NAME,
            nvl(SCR.M_PROCESS_NB, 0) as SCANNER_ENGINES,
            DECODE(nvl(SCR.M_BATCH_SIZE,-1),-1,0, 2000000000 , 0, 0, M_THRESHOLD, M_BATCH_SIZE) as SCANNER_ENGINE_SIZE,
            DECODE(nvl(SCR.M_BATCH_SIZE,-1), -1,'', 0, 'By Threshold',2000000000, 'By positions in one batch','By Size') as SCANNER_ENGINE_TYPE,
            decode(nvl(SCR.M_RETRIES_BATCH_SIZE,-1),-1,0, 2000000000,0,M_RETRIES_BATCH_SIZE) as SCANNER_ENGINES_RETRY_TIME,
            decode(nvl(SCR.M_RETRIES_THRESHOLD,-1),-1,0,M_RETRIES_THRESHOLD) as SCANNER_RETRIES_THRESHOLD,
